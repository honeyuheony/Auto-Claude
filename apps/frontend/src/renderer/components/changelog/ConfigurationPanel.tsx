      ...
      <div className="p-6 space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg bg-muted/50 p-3">
            <div className="flex items-center gap-2 text-sm font-medium">
              ...
              {summaryInfo.count === 1
                ? t('changelog.includingCount', { count: summaryInfo.count, label: summaryInfo.label })
                : t('changelog.includingCountPlural', { count: summaryInfo.count, label: summaryInfo.label })}
            </div>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm">{t('changelog.releaseInfo')}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="date">{t('changelog.date')}</Label>
              <Input
                id="date"
                type="date"
                value={date}
            ...
            {(existingChangelog?.lastVersion || versionBumpDescription) && (
              <div className="text-xs text-muted-foreground space-y-1">
                {existingChangelog?.lastVersion && (
                  <p>{t('changelog.previousVersion', { version: existingChangelog.lastVersion })}</p>
                )}
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm">{t('changelog.outputStyle')}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label>{t('changelog.format')}</Label>
              <Select
                value={format}
            ...
            <div className="space-y-2">
              <Label>{t('changelog.audience')}</Label>
              <Select
                value={audience}
            ...
            <div className="space-y-2">
              <Label>{t('changelog.emojis')}</Label>
              <Select
                value={emojiLevel}
          ...
          <CollapsibleContent className="pt-2">
            <Card>
              <CardContent className="pt-4">
                <div className="space-y-2">
                  <Label htmlFor="instructions">{t('changelog.customInstructions')}</Label>
                  <Textarea
                    id="instructions"
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t('changelog.customInstructionsHint')}
                  </p>
                </div>
              </CardContent>
            </Card>
