      ...
      window.electronAPI.getIssueComments(projectId, selectedIssue.number)
        ...
        .catch((err: unknown) => {
          ...
          setFetchCommentsError(
            err instanceof Error ? err.message : t('tasks:githubIssues.failedToLoadComments')
          );
        })
        .finally(() => {
          if (isMounted) {
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {t('tasks:githubIssues.createTaskFromIssue')}
          </DialogTitle>
          <DialogDescription>
            {selectedIssue && (
              <span>
        ...
        {investigationStatus.phase === 'idle' ? (
          <div className="space-y-4 flex-1 min-h-0 flex flex-col">
            <p className="text-sm text-muted-foreground">
              {t('tasks:githubIssues.createTaskDescription')}
            </p>
            ...
            {loadingComments ? (
              ...
              <div className="rounded-lg bg-destructive/10 border border-destructive/30 p-4">
                <p className="text-sm text-destructive font-medium">{t('tasks:githubIssues.failedToLoadComments')}</p>
              ...
              <div className="space-y-2 flex-1 min-h-0 flex flex-col">
                <div className="flex items-center justify-between">
                  <h4 className="text-sm font-medium flex items-center gap-2">
                    ...
                    {t('tasks:githubIssues.selectCommentsToInclude', { selected: selectedCommentIds.length, total: comments.length })}
                  </h4>
                  <Button
                    variant="ghost"
                    size="sm"
                    ...
                    className="text-xs"
                  >
                    {selectedCommentIds.length === comments.length ? t('tasks:githubIssues.deselectAll') : t('tasks:githubIssues.selectAll')}
                  </Button>
                </div>
              ...
              <div className="rounded-lg border border-border bg-muted/30 p-4">
                <h4 className="text-sm font-medium mb-2">{t('tasks:githubIssues.taskWillInclude')}</h4>
                <ul className="text-sm text-muted-foreground space-y-1">
                  <li>• {t('tasks:githubIssues.issueTitleAndDescription')}</li>
                  <li>• {t('tasks:githubIssues.linkBackToGithubIssue')}</li>
                  <li>• {t('tasks:githubIssues.labelsAndMetadata')}</li>
                  <li>• {t('tasks:githubIssues.noComments')}</li>
                </ul>
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            ...
            {investigationStatus.phase === 'complete' && (
              <div className="rounded-lg bg-success/10 border border-success/30 p-3 flex items-center gap-2 text-sm text-success">
                ...
                {t('tasks:githubIssues.taskCreated')}
              </div>
            )}
          </div>
        )}
        ...
        <DialogFooter>
          {investigationStatus.phase === 'idle' && (
            <>
              ...
              <Button onClick={handleStartInvestigation}>
                ...
                {t('tasks:githubIssues.createTask')}
              </Button>
            </>
          )}
          {investigationStatus.phase !== 'idle' && investigationStatus.phase !== 'complete' && (
            <Button variant="outline" disabled>
              ...
              {t('tasks:githubIssues.creating')}
            </Button>
          )}
          {investigationStatus.phase === 'complete' && (
            <Button onClick={onClose}>
              {t('tasks:githubIssues.done')}
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
